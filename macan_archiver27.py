...
# Dialog diupdate untuk menyertakan input password
# Dialog diupdate untuk menyertakan input password, SFX, dan split
class ArchiveSettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # --- OPSI BARU ---
        self.sfx_check = QCheckBox("Create SFX archive (.exe)") # BARU
        layout.addRow("", self.sfx_check) # BARU

        self.split_edit = QLineEdit() # BARU
        self.split_edit.setPlaceholderText("e.g., 100m, 2g (optional)") # BARU
        layout.addRow(QLabel("Split into volumes:"), self.split_edit) # BARU
        # --- AKHIR OPSI BARU ---
    ...
    def get_settings(self):
        ...
        is_sfx = self.sfx_check.isChecked() # BARU: Ambil status checkbox SFX
        split_size = self.split_edit.text().strip() # BARU: Ambil ukuran split
        return level, password, is_sfx, split_size # BARU: Kembalikan nilai baru
...
# Worker untuk operasi arsip menggunakan 7z.exe (Create, Add/Update)
class ArchiveWorker(QObject):
    # --- BARU: Sinyal untuk melaporkan progres ---
    progress = Signal(int)        # Sinyal untuk persentase (0-100)
    progress_text = Signal(str)   # Sinyal untuk teks (misal: "Compressing file.txt")
    finished = Signal(str)
    error = Signal(str)

    def __init__(self, sevenzip_path, command, archive_path, source_paths, level, password, is_sfx, split_size):
        super().__init__()
        ...
        self.is_sfx = is_sfx # BARU
        self.split_size = split_size # BARU
    ...
    def run(self):
        try:
            ...
            # --- LOGIKA BARU ---
            if self.is_sfx:
                cmd.append('-sfx') # Tambahkan switch SFX jika dicentang
            if self.split_size:
                # Validasi sederhana untuk format (contoh: 10m, 1g, 500k)
                if re.match(r'^\d+[kmgKMG]?$', self.split_size):
                    cmd.append(f'-v{self.split_size}') # Tambahkan switch volume/split
            # --- AKHIR LOGIKA BARU ---
            ...
            # --- DIUBAH: Jalankan proses dan baca outputnya secara real-time ---
            self.process = subprocess.Popen(
                cmd,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                stdin=subprocess.PIPE, # Mencegah 7z menunggu input
                startupinfo=startupinfo,
                # Tambahkan flag ini untuk menghindari pembuatan jendela konsol
                creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0,
                text=True,
                encoding='utf-8',
                errors='ignore' # Mengabaikan error encoding jika ada
            )

            # Baca output stdout baris per baris untuk mendapatkan progres
            for line in iter(self.process.stdout.readline, ''):
                # Cek apakah pengguna menekan tombol cancel
                if not self._is_running:
                    self.process.terminate() # Hentikan proses 7z
                    break
                
                # Parsing persentase progres menggunakan regular expression
                # 7-Zip biasanya mencetak sesuatu seperti " 25% ..."
                match = re.search(r'(\d+)\s*%', line)
                if match:
                    percentage = int(match.group(1))
                    self.progress.emit(percentage) # Kirim sinyal persentase
                
                # Parsing nama file yang sedang diproses
                if line.startswith("Compressing"):
                    self.progress_text.emit(line.strip()) # Kirim sinyal teks

            self.process.stdout.close()
            return_code = self.process.wait()

            # Jika dibatalkan, kirim sinyal error
            if not self._is_running:
                self.error.emit("Canceled")
                return

            if return_code == 0:
                self.finished.emit(self.archive_path) # Sukses
            else:
                # Jika gagal, baca error dari stderr
    ...
    # --- BARU: Fungsi untuk menghentikan worker ---
    def stop(self):
        self._is_running = False
        if self.process:
            try:
                # Coba hentikan proses dengan baik-baik
                self.process.terminate()
            except ProcessLookupError:
                # Proses mungkin sudah selesai, abaikan saja
                pass
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        self.sevenzip_path = self._find_7z_executable() # Cari 7z.exe saat startup
        ...
        if not self.sevenzip_path:
             QMessageBox.critical(self, "7-Zip Not Found",
                ...
                "Please install 7-Zip and ensure its directory is in your PATH, or place it alongside the application executable.\n"
    ...
    # Fungsi untuk menemukan 7z.exe
    def _find_7z_executable(self):
        """Finds the 7-Zip executable, prioritizing the bundled version."""
        # Prioritaskan path dari _MEIPASS jika aplikasi di-bundle
        if hasattr(sys, "_MEIPASS"):
            bundled_path = os.path.join(sys._MEIPASS, "7z.exe")
    ...
    # Metode _create_archive menggunakan worker baru
    def _perform_archive_creation(self, source_paths, update_mode=False):
        if not source_paths: return
        
        if update_mode:
            # Fitur SFX dan Split tidak berlaku untuk mode 'update' (add files)
            ...
            # --- DIUBAH: Dialog file dan pengaturan ---
            ...
            # Tentukan filter dan ekstensi default berdasarkan pilihan SFX
            if is_sfx:
            ...
            # Otomatis perbaiki ekstensi jika perlu
        ...
        # --- DIUBAH: Kirim parameter baru ke Worker ---
        self.worker = ArchiveWorker(self.sevenzip_path, command, save_path, source_paths, level, password, is_sfx, split_size)
    ...
    # --- FUNGSI BARU: Handler saat operasi arsip selesai ---
    def _on_archive_operation_finished(self, save_path):
        self.progress_dialog.setValue(self.progress_dialog.maximum()) # Set progress ke 100%
        self.progress_dialog.close()
        self.status_bar.showMessage(f"Operation on {os.path.basename(save_path)} successful!", 5000)
        self._list_archive_contents(save_path, force_reload=True) # Muat ulang isi arsip
        self.thread.quit() # Hentikan thread dengan aman

    # --- FUNGSI BARU: Handler saat terjadi error atau pembatalan ---
    ...
    # Fungsi untuk mendaftar isi arsip menggunakan '7z l -slt'
    ...
    # [DIREVISI] Fungsi _extract_archive menjadi lebih simpel, tanpa progress bar
    def _extract_archive(self, selected_only=False, copy_mode=False):
        ...
        if self._is_archive_encrypted(self.current_archive_path):
            ...
            if password is None: # User membatalkan dialog password
                self.status_bar.showMessage("Extraction canceled.", 3000)
                return
        
        self.status_bar.showMessage("Extracting, please wait...")
        QApplication.setOverrideCursor(Qt.WaitCursor) # Ubah cursor jadi loading
        ...
        try:
            ...
            # Jalankan proses secara blocking (menunggu sampai selesai)
            ...
            if result.returncode == 0:
                ...
                # Tampilkan error jika proses 7z gagal
                raise RuntimeError(result.stderr or "An unknown error occurred with 7-Zip.")

        except Exception as e:
            QApplication.restoreOverrideCursor() # Pastikan cursor kembali normal jika ada error
    ...
    # [DIHAPUS] Slot _on_extraction_finished dan _on_extraction_error tidak diperlukan lagi

    # Fungsi untuk menambah file ke arsip yang ada
    ...
    # _test_archive menggunakan 7z.exe
    ...
    # _delete_selected menggunakan 7z.exe 'd' command
    ...
    def _show_help_content(self):
        ...
        <ul>
            <li><b>Creating Archives:</b> Click the 'Create' icon or use File > Create Archive. You can select multiple files or a single folder. In the settings dialog, you can choose compression level and set an optional password.</li>
            ...
            <li><b>Password Protection:</b> Macan Archiver can create and open password-protected archives, powered by 7-Zip's strong AES-256 encryption.</li>
        </ul>
