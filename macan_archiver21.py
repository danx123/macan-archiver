...
# Dialog Setting Arsip dengan Opsi Password
        ...
        try:
            ...
            if self.archive_type == '7z':
                with py7zr.SevenZipFile(self.archive_path, 'r', password=self.password) as archive:
                    # [FIX] Iterate and extract one-by-one to keep the GUI responsive
                    # This is slower but prevents the "Not Responding" state.
...
class MacanArchiver(QMainWindow):
    ...
    def _create_menus_and_toolbars(self):
        ...
        self.add_files_action = QAction(self._get_themed_icon("add_files"), "&Add Files...", self)
        ...
        self.add_files_action.triggered.connect(self._add_files_to_archive)
    ...
    def _list_archive_contents(self, file_path, force_reload=False, password=None):
        ...
        try:
            ...
            self.test_action.setEnabled(True) # Enabled for all types, test logic will handle specifics
    ...
    def _show_context_menu(self, pos):
        ...
        if self.current_archive_path:
            ...
            elif action and action == add_files_action: self._add_files_to_archive()
    ...
    def _test_archive(self):
        ...
        try:
            if self.current_archive_type == '7z':
                with py7zr.SevenZipFile(self.current_archive_path, 'r', password=self.current_archive_password) as archive: 
                    archive.test()
            elif self.current_archive_type == 'zip':
                with zipfile.ZipFile(self.current_archive_path, 'r') as archive:
                    if self.current_archive_password:
                        archive.setpassword(self.current_archive_password.encode('utf-8'))
                    archive.testzip()
            elif self.current_archive_type == 'rar' and RAR_SUPPORT:
                with rarfile.RarFile(self.current_archive_path, 'r') as archive:
                    if self.current_archive_password:
                        archive.setpassword(self.current_archive_password)
                    archive.testrar()
            else:
                 QMessageBox.information(self, "Unsupported", f"Testing is not supported for '{self.current_archive_type}' archives.")
                 self.status_bar.showMessage("Test unsupported for this archive type.", 5000)
                 return
            test_passed = True
        except (py7zr.PasswordRequired, rarfile.PasswordRequired, RuntimeError) as e:
            error_details = "Cannot test archive. The password is required or incorrect."
        except Exception as e:
            error_details = f"Archive is corrupted or invalid.\n\nDetails: {e}"
        ...
        if test_passed:
            ...
            QMessageBox.critical(self, "Test Failed", error_details)
    ...
    def _perform_extraction(self, dest_folder, targets=None, show_success_msg=True, finish_callback=None, error_callback=None):
        ...
        self.thread = QThread()
        self.worker = ExtractionWorker(self.current_archive_path, self.current_archive_type, dest_folder, file_list, self.current_archive_password)
        self.worker.moveToThread(self.thread)
    ...
    def _show_about(self):
        ...
        QMessageBox.information(self, "About Macan Archiver",
            "<b>Macan Archiver v3.1.1 (Fixed)</b><br><br>"
            ...
            "<b>Features:</b> Create/Open password-protected archives, Non-blocking delete & extract.<br><br>"
            f"{rar_note}<br><br>"
