...
# [BARU] Worker class untuk proses ekstraksi di background thread
class ExtractionWorker(QObject):
    ...
    def __init__(self, archive_type, archive_path, dest_folder, targets=None):
        super().__init__()
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        # Inisialisasi QSettings
        self.settings = QSettings("MacanAngkasa", "MacanArchiver")

        # Palet warna untuk ikon tema terang dan gelap
        self.theme_colors = {
            'dark': {
        ...
        self.current_theme = 'dark' # Default sebelum memuat settings
        
        # Inisialisasi awal, akan dioverride oleh _load_settings
        self.current_directory = os.path.expanduser("~") 
        
        self.total_uncompressed_size = 0
        self.total_files_in_archive = 0
        
        # [BARU] Timer dan properti untuk melacak waktu proses
        ...
        self._setup_ui()
        self._load_settings() # Memuat settings saat startup
    ...
    def _get_themed_icon(self, icon_name, icon_type='toolbar'):
        # 'archive' icon tidak memiliki placeholder warna, jadi kembalikan langsung
    ...
    def _load_settings(self):
        # Memuat Geometri Jendela
        geometry = self.settings.value("geometry")
        if geometry:
            self.restoreGeometry(geometry)
        else:
            self.setGeometry(100, 100, 800, 600) # Fallback default

        # [BARU] Memuat state jendela (termasuk posisi toolbar)
        window_state = self.settings.value("main_window_state")
        if window_state:
            self.restoreState(window_state)

        # Memuat Tema
        theme = self.settings.value("theme", "dark") # Default ke 'dark'
        self._set_theme(theme)

        # Memuat Path Terakhir
    ...
    # Fungsi untuk mengganti tema dan memperbarui ikon
    ...
    # Fungsi untuk memperbarui semua ikon saat tema berubah
    def _update_all_icons(self):
        # Perbarui ikon di semua action
        ...
        # Muat ulang tampilan tabel untuk memperbarui ikon file/folder
    ...
    def _create_menus_and_toolbars(self):
        ...
        self.info_action = self._get_themed_icon("info") # Simpan QIcon untuk about
        ...
        nav_toolbar.setObjectName("NavigationToolbar") # [BARU] Beri nama objek untuk disimpan
        ...
        # [MODIFIKASI] Mengubah urutan widget di toolbar navigasi
        ...
        main_toolbar.setObjectName("MainToolbar") # [BARU] Beri nama objek untuk disimpan
    ...
    # closeEvent untuk menyimpan settings
    def closeEvent(self, event):
        # Menyimpan settings
        self.settings.setValue("geometry", self.saveGeometry())
        self.settings.setValue("last_path", self.current_directory)
        self.settings.setValue("theme", self.current_theme)
        # [BARU] Menyimpan state jendela (posisi toolbar, dll)
    ...
    # [BARU] Fungsi untuk mengupdate info waktu di progress dialog
    def _update_progress_time_info(self):
        if not self.operation_start_time or not hasattr(self, 'progress_dialog'):
            return
        ...
        elapsed_str = format_timedelta(elapsed)
        
        current_val = self.progress_dialog.value()
        max_val = self.progress_dialog.maximum()
    ...
    def _perform_archive_creation(self, source_paths):
        ...
        self.worker.progress_text.connect(self._set_current_operation_text) # [MODIFIKASI]
        
        self.operation_start_time = datetime.now() # [BARU]
        self.progress_timer.start(1000) # [BARU] Update tiap detik
        self.thread.start()
        self.progress_dialog.exec()

    def _on_archive_creation_finished(self, save_path):
        self.progress_timer.stop() # [BARU]
    ...
    def _list_archive_contents(self, file_path, force_reload=False):
        # Hindari reload jika path sama dan tidak dipaksa
    ...
    def _get_icon_for_file(self, name, is_dir):
        if is_dir:
            return self._get_themed_icon("folder", "folder")
        
        ext = os.path.splitext(name)[1].lower()
        if ext in ['.zip', '.7z', '.rar', '.tar', '.gz', '.bz2', '.xz', '.mcn']:
            return self._get_themed_icon("archive")
        if ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.svg']:
            return self._get_themed_icon("image", "image")
        if ext in ['.txt', '.md', '.log', '.py', '.js', '.html', '.css', '.json', '.xml']:
            return self._get_themed_icon("text", "text")
    ...
    # [BARU] Fungsi terpusat untuk menjalankan proses ekstraksi
    def _perform_extraction(self, targets=None, dest_folder=None, show_success_msg=True):
        ...
        self.progress_dialog.setWindowTitle("Extracting Archive")
        self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
        self.progress_dialog.setAutoClose(False)
        self.progress_dialog.setAutoReset(False)

        self.thread = QThread()
        self.worker = ExtractionWorker(self.current_archive_type, self.current_archive_path, dest_folder, targets)
        self.worker.moveToThread(self.thread)

        self.progress_dialog.canceled.connect(self.worker.stop)
        self.thread.started.connect(self.worker.run)
        
        # [MODIFIKASI] Lambda untuk menangani argumen tambahan
    ...
    # [BARU] Slot untuk menangani selesai ekstraksi
    ...
    # [BARU] Slot untuk menangani error ekstraksi
    ...
    def _handle_item_double_click(self, item):
        ...
        if self.current_archive_path: # Mode Arsip
            ...
            QApplication.processEvents()
            try:
                # [MODIFIKASI] Menggunakan fungsi ekstraksi yang baru
    ...
    def _extract_or_copy_selected(self):
        ...
        # [MODIFIKASI] Memanggil fungsi ekstraksi terpusat
