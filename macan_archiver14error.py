...
# Modul 'winreg' untuk integrasi shell menu di Windows
try:
    import rarfile
    RAR_SUPPORT = True
except ImportError:
...
from PySide6.QtWidgets import (
    ...
    QMenu, QProgressDialog, QLineEdit, QProgressBar, QMenuBar, QCheckBox,
    QTextEdit # [PERBAIKAN] Impor QTextEdit untuk help content
)
from PySide6.QtGui import QAction, QIcon, QPixmap, QDesktopServices
from PySide6.QtCore import Qt, QSize, QByteArray, QTimer, QThread, Signal, QObject, QUrl, QSettings

# --- [MODIFIKASI] Menambahkan placeholder "{color}" untuk SVG Ikon ---
SVG_ICONS = {
...
QLineEdit { padding: 4px; border: 1px solid #555555; border-radius: 3px; background-color: #333333; }
...
QMessageBox QLabel, QDialog QLabel, QDialog QComboBox, QDialog QProgressBar, QDialog QCheckBox, QTextEdit { color: #e0e0e0; background-color: #2b2b2b; }
...
# [BARU] Dialog untuk memasukkan password
class PasswordDialog(QDialog):
    def __init__(self, parent=None, filename=""):
        ...
        self.label = QLabel(f"Enter password for:\n<b>{os.path.basename(filename)}</b>")
...
class ArchiveWorker(QObject):
    ...
    def __init__(self, source_paths, save_path, filters, password=None):
        super().__init__()
    ...
    def run(self):
        try:
            with py7zr.SevenZipFile(self.save_path, 'w', filters=self.filters, password=self.password) as archive:
                processed_files = 0
...
class ExtractionWorker(QObject):
    ...
    def __init__(self, archive_type, archive_path, dest_folder, targets=None, password=None):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if self.archive_type == '7z':
                with py7zr.SevenZipFile(self.archive_path, 'r', password=self.password) as archive:
                    files_to_extract = self.targets or archive.getnames()
                    # [PERBAIKAN] Panggil extract sekali saja, bukan dalam loop. Ini lebih stabil.
                    self.progress_text.emit(f"Extracting {len(files_to_extract)} items...")
                    archive.extract(path=self.dest_folder, targets=files_to_extract)
                    self.progress.emit(len(files_to_extract))

            elif self.archive_type == 'zip':
                pwd_bytes = self.password.encode('utf-8') if self.password else None
                with zipfile.ZipFile(self.archive_path, 'r') as archive:
                    # Menentukan file yang akan diekstrak (semua atau yang ditargetkan)
                    files_to_extract = self.targets or [info.filename for info in archive.infolist()]
                    
                    # [PERBAIKAN] Menggunakan extractall untuk efisiensi dan stabilitas
                    # Ini mencegah UI freeze (not responding) dengan memanggil satu operasi besar
                    # daripada banyak operasi kecil di dalam perulangan.
                    self.progress_text.emit(f"Extracting {len(files_to_extract)} items...")
                    
                    # 'members' memastikan hanya file yang dipilih yang diekstrak
                    archive.extractall(path=self.dest_folder, members=files_to_extract, pwd=pwd_bytes)
                    
                    # Update progress bar setelah selesai
                    self.progress.emit(len(files_to_extract))

            elif self.archive_type == 'tar':
                with tarfile.open(self.archive_path, 'r:*') as archive:
                    members_to_extract = [m for m in archive.getmembers() if m.name in self.targets] if self.targets else archive.getmembers()
                    # [PERBAIKAN] Panggil extractall sekali saja untuk konsistensi dan efisiensi
                    self.progress_text.emit(f"Extracting {len(members_to_extract)} items...")
                    archive.extractall(path=self.dest_folder, members=members_to_extract)
                    self.progress.emit(len(members_to_extract))
            
            elif self.archive_type == 'rar' and RAR_SUPPORT:
                with rarfile.RarFile(self.archive_path, 'r', pwd=self.password) as archive:
                    files_to_extract = self.targets or [info.filename for info in archive.infolist()]
                    # [PERBAIKAN] Panggil extractall sekali saja, lebih robust.
                    self.progress_text.emit(f"Extracting {len(files_to_extract)} items...")
                    archive.extractall(path=self.dest_folder, members=files_to_extract)
                    self.progress.emit(len(files_to_extract))

            if self._is_running:
                self.finished.emit(f"Successfully extracted to {self.dest_folder}")
            else:
                self.error.emit("Canceled")
        except Exception as e:
            # Periksa secara spesifik untuk error password
            if "password" in str(e).lower() or isinstance(e, py7zr.PasswordRequired) or (isinstance(e, rarfile.BadRarFile) and "password" in str(e).lower()):
                 self.error.emit("Password incorrect or required.") # Kirim pesan error yang lebih spesifik
            else:
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        self.current_archive_password = None
        ...
        self.progress_timer.timeout.connect(self._update_progress_time_info)
        self.operation_start_time = None
        self.current_operation_text = ""
    ...
    def _update_all_icons(self):
        ...
        if self.current_archive_path:
            self._list_archive_contents(self.current_archive_path, force_reload=True, password_attempt=self.current_archive_password)
        else:
        ...
        try:
            for name in sorted(os.listdir(path), key=str.lower):
                ...
                try:
                    ...
                    items.append({ 'name': name, 'size': stat.st_size, 'modified': datetime.fromtimestamp(stat.st_mtime).strftime('%Y-%m-%d %H:%M:%S'), 'is_dir': is_dir })
    ...
    def _update_progress_time_info(self):
        if not self.operation_start_time or not hasattr(self, 'progress_dialog'): return
        ...
        elapsed = datetime.now() - self.operation_start_time; elapsed_str = format_timedelta(elapsed)
        current_val, max_val = self.progress_dialog.value(), self.progress_dialog.maximum()
        remaining_str = "..."
        if current_val > 0 and elapsed.total_seconds() > 1:
            rate = current_val / elapsed.total_seconds()
            remaining_seconds = (max_val - current_val) / rate
            remaining_str = f"~{format_timedelta(timedelta(seconds=remaining_seconds))}"

        self.progress_dialog.setLabelText(f"{self.current_operation_text}\nElapsed: {elapsed_str} | Remaining: {remaining_str}")

    def _set_current_operation_text(self, text):
        self.current_operation_text = text
    ...
    def _perform_archive_creation(self, source_paths):
        ...
        if not save_path:
            self.status_bar.showMessage("Canceled archive creation."); return

        settings_dialog = ArchiveSettingsDialog(self)
        if not settings_dialog.exec():
            self.status_bar.showMessage("Canceled archive creation."); return
            
        if not save_path.endswith(('.mcn', '.7z')): save_path += '.mcn'

        method, level, password = settings_dialog.get_settings()
        ...
        self.progress_dialog.setWindowTitle("Creating Archive"); self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
        self.progress_dialog.setAutoClose(False); self.progress_dialog.setAutoReset(False)
        
        self.thread = QThread()
        self.worker = ArchiveWorker(source_paths, save_path, filters, password)
        ...
        self.worker.progress_text.connect(self._set_current_operation_text)
        
        self.operation_start_time = datetime.now(); self.progress_timer.start(1000)
        self.thread.start()
        self.progress_dialog.show() # Ganti .exec() menjadi .show()
    ...
    def _on_archive_creation_error(self, message):
        self.progress_timer.stop(); self.progress_dialog.close()
        if message == "Canceled":
            self.status_bar.showMessage("Archive creation canceled.", 5000)
            if hasattr(self.worker, 'save_path') and os.path.exists(self.worker.save_path): os.remove(self.worker.save_path)
        else:
    ...
    def _open_archive(self):
        ...
        if file_path: self._list_archive_contents(file_path)

    def _list_archive_contents(self, file_path, force_reload=False, password_attempt=None):
        if self.current_archive_path == file_path and not force_reload: return
        ...
        try:
            if file_ext in ['.mcn', '.7z']:
                ...
                files_info = self._list_7z_contents(file_path, password_attempt)
            elif file_ext == '.zip':
                self.current_archive_type = 'zip'
                files_info = self._list_zip_contents(file_path, password_attempt)
            elif file_ext == '.rar':
                if not RAR_SUPPORT: raise ImportError("Module 'rarfile' is required for .rar support.")
                self.current_archive_type = 'rar'
                files_info = self._list_rar_contents(file_path, password_attempt)
            ...
            self.current_archive_password = password_attempt
        ...
        except (py7zr.PasswordRequired, RuntimeError, rarfile.BadRarFile, zipfile.BadZipFile, zipfile.LargeZipFile) as e:
            if "password" in str(e).lower() or isinstance(e, py7zr.PasswordRequired) or (isinstance(e, rarfile.BadRarFile) and "Bad password" in str(e)):
                dialog = PasswordDialog(self, file_path)
                if dialog.exec():
                    self._list_archive_contents(file_path, force_reload=True, password_attempt=dialog.get_password())
                else: 
                    self._browse_to_path(os.path.dirname(file_path) or self.current_directory)
            else:
    ...
    def _list_7z_contents(self, file_path, password=None):
    ...
    def _list_zip_contents(self, file_path, password=None):
        pwd_bytes = password.encode('utf-8') if password else None
        with zipfile.ZipFile(file_path, 'r') as archive:
            # Need to test read to trigger password error
            try: archive.read(archive.infolist()[0].filename, pwd=pwd_bytes)
            except (RuntimeError, zipfile.BadZipFile) as e:
                if "password" in str(e).lower(): raise e # Re-raise for password dialog
    ...
    def _list_rar_contents(self, file_path, password=None):
        with rarfile.RarFile(file_path, 'r', pwd=password) as archive:
    ...
    def _extract_archive(self):
        if not self.current_archive_path: return
        self._perform_extraction(targets=None)

    def _perform_extraction(self, targets=None, dest_folder=None, show_success_msg=True):
        if dest_folder is None:
            dest_folder = QFileDialog.getExistingDirectory(self, "Select Extraction Destination", self.current_directory)
        if not dest_folder:
            self.status_bar.showMessage("Extraction canceled.", 3000); return

        total_files = len(targets) if targets else self.total_files_in_archive
        self.progress_dialog = QProgressDialog("Starting extraction...", "Cancel", 0, total_files, self)
        self.progress_dialog.setWindowTitle("Extracting Archive"); self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
        self.progress_dialog.setAutoClose(False); self.progress_dialog.setAutoReset(False)

        self.thread = QThread()
        self.worker = ExtractionWorker(self.current_archive_type, self.current_archive_path, dest_folder, targets, self.current_archive_password)
        self.worker.moveToThread(self.thread)

        self.progress_dialog.canceled.connect(self.worker.stop)
        self.thread.started.connect(self.worker.run)
        
        finish_slot = lambda msg: self._on_extraction_finished(msg, show_success_msg)
    ...
    def _on_extraction_finished(self, message, show_success_msg):
        self.progress_timer.stop()
        self.progress_dialog.setValue(self.progress_dialog.maximum()); self.progress_dialog.close()
        self.status_bar.showMessage("Extraction completed successfully!", 5000)
        if show_success_msg: QMessageBox.information(self, "Success", message)
        #self.thread.quit(); self.thread.wait()

    def _on_extraction_error(self, message):
        self.progress_timer.stop(); self.progress_dialog.close()
        if message == "Canceled": self.status_bar.showMessage("Extraction canceled.", 5000)
        # [PERBAIKAN] Tangani permintaan password saat ekstraksi
        elif "Password incorrect or required." in message:
            dialog = PasswordDialog(self, self.current_archive_path)
            if dialog.exec():
                self.current_archive_password = dialog.get_password()
                # Coba lagi ekstraksi dengan password baru
                self._perform_extraction(targets=self.worker.targets, dest_folder=self.worker.dest_folder, show_success_msg=True)
            else:
                self.status_bar.showMessage("Extraction canceled, password not provided.", 5000)
        else:
            QMessageBox.critical(self, "Extraction Error", message); self.status_bar.showMessage("Extraction failed.", 5000)
        self.thread.quit(); self.thread.wait()
    ...
    def _handle_item_double_click(self, item):
        ...
        if self.current_archive_path:
            ...
            # Cek jika file sudah ada di temp dir
            ...
            try:
                self._perform_extraction(targets=[item_name], dest_folder=self.temp_dir, show_success_msg=False)
                if os.path.exists(extracted_file_path):
                    QDesktopServices.openUrl(QUrl.fromLocalFile(extracted_file_path))
                    self.status_bar.showMessage(f"Opened {item_name}", 3000)
            except Exception as e:
    ...
    def _show_context_menu(self, pos):
        ...
        if self.current_archive_path:
            ...
            if self.current_archive_type == '7z': context_menu.addSeparator(); delete_action = context_menu.addAction("Delete Selected")
    ...
    def _test_archive(self):
        ...
        try:
            with py7zr.SevenZipFile(self.current_archive_path, 'r', password=self.current_archive_password) as archive: archive.test()
            ...
            if "password" in str(e).lower():
                QMessageBox.warning(self, "Test Failed", "Password may be incorrect. Please re-open the archive with the correct password.")
            else:
    ...
    def _extract_or_copy_selected(self):
        ...
        if not filenames:
            QMessageBox.warning(self, "No Selection", "Please select files or folders to extract/copy."); return
        self._perform_extraction(targets=filenames)
    ...
    def _delete_selected(self):
        ...
        try:
            with py7zr.SevenZipFile(original_path, 'r', password=self.current_archive_password) as archive_in:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with py7zr.SevenZipFile(temp_archive_path, 'w', password=self.current_archive_password) as archive_out:
    ...
    def _show_about(self):
        ...
        QMessageBox.information(self, "About Macan Archiver",
            "<b>Macan Archiver v3.1.0 (Guardian Edition)</b><br><br>"
            "A professional file archiver with theme support, file icons, CLI, password protection, and Windows Shell integration. 🧭<br>"
    ...
    # [PERBAIKAN] Menggunakan QDialog dengan QTextEdit agar bisa di-scroll
    ...
    def _update_shell_integration(self, register=True):
        ...
        try:
            ...
            archive_commands = {
                ...
                r'shell\MacanExtractHere': ("Extract Here (Macan)", f'{base_command} --extract-here "%1"'),
                r'shell\MacanExtractTo': ("Extract To... (Macan)", f'{base_command} --extract-to "%1"'),
            }
            ...
            add_to_archive_cmd = f'{base_command} --add-to-archive %*'
            target_keys = {
            ...
            for key_path, value in target_keys.items():
                if register:
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, key_path) as key:
                        winreg.SetValue(key, None, winreg.REG_SZ, value)
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, fr'{key_path}\command') as cmd_key:
                        winreg.SetValue(cmd_key, None, winreg.REG_SZ, add_to_archive_cmd)
                else:
                    try:
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, fr'{key_path}\command')
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, key_path)
...
# --- [BAGIAN BARU] Fungsi untuk menangani argumen CLI dari shell integration ---

def handle_cli_add_to_archive(source_paths):
    """Memulai proses pembuatan arsip dari CLI."""
    app = QApplication.instance() or QApplication(sys.argv)
    main_window = MacanArchiver()
    main_window.current_directory = os.path.dirname(source_paths[0])
    main_window._perform_archive_creation(source_paths)

def handle_cli_extract(archive_path, extract_here=False):
    """Memulai proses ekstraksi dari CLI."""
    app = QApplication.instance() or QApplication(sys.argv)
    main_window = MacanArchiver()

    dest_folder = None
    if extract_here:
        dest_folder = os.path.dirname(archive_path)
    else:
        dest_folder = QFileDialog.getExistingDirectory(None, "Select Extraction Destination", os.path.dirname(archive_path))
    ...
    archive_type_map = {'.mcn': '7z', '.7z': '7z', '.zip': 'zip', '.rar': 'rar'}
    archive_type = archive_type_map.get(file_ext, 'tar' if tarfile.is_tarfile(archive_path) else None)
    
    if not archive_type:
        QMessageBox.critical(None, "Error", "Unsupported archive type for CLI extraction.")
        return

    password = None
    while True:
        worker = ExtractionWorker(archive_type, archive_path, dest_folder, password=password)
        thread = QThread()
        ...
        # Jalankan secara sinkron
        thread.start()
        while thread.isRunning():
            app.processEvents()
        
        if success:
            QMessageBox.information(None, "Success", f"Successfully extracted to {dest_folder}")
            break
        elif "Password" in error_message:
            dialog = PasswordDialog(filename=archive_path)
            if dialog.exec():
                password = dialog.get_password()
            else:
                print("Password not provided. Extraction canceled.")
                break
        else:
            QMessageBox.critical(None, "Extraction Error", f"An error occurred: {error_message}")
            break
...
def handle_cli_creation(args):
    ...
    try:
        with py7zr.SevenZipFile(args.output, 'w', filters=filters, password=args.password) as archive:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', nargs='+', help='Source files/directories for --create.')
    parser.add_argument('--output', help='Output archive for --create.')
    ...
    parser.add_argument('--password', help='Set password for the archive (CLI mode).')
    
    parser.add_argument('--add-to-archive', nargs='+', help='(Shell) Add specified files/folders to an archive.')
    parser.add_argument('--extract-here', help='(Shell) Extract archive to its current directory.')
    parser.add_argument('--extract-to', help='(Shell) Open dialog to extract archive.')
    
    parser.add_argument('file_path_arg', nargs='?', default=None, help='File to open directly in the GUI.')
    
    args, unknown = parser.parse_known_args()

    if args.add_to_archive:
        handle_cli_add_to_archive(args.add_to_archive)
        sys.exit(0)
    elif args.extract_here:
        handle_cli_extract(args.extract_here, extract_here=True)
        sys.exit(0)
    elif args.extract_to:
        handle_cli_extract(args.extract_to, extract_here=False)
        sys.exit(0)
    elif args.create:
    ...
    if not file_to_open and unknown:
        if os.path.exists(unknown[0]):
            file_to_open = unknown[0]
