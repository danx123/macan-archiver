...
# (Tidak ada perubahan di bagian ini, jadi disembunyikan untuk keringkasan)
SVG_ICONS = {
...
QComboBox { background-color: #2b2b2b; border: 1px solid #555555; padding: 4px; border-radius: 3px; }
...
# [BARU] Worker untuk proses pembuatan arsip di thread terpisah
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        # [BARU] Membuat direktori temporary untuk membuka file
    ...
    def _setup_ui(self):
        ...
        # [BARU] Hubungkan sinyal double-click untuk membuka file
        self.table_widget.itemDoubleClicked.connect(self._open_item_in_archive)
    ...
    # [BARU] Membersihkan folder temporary saat aplikasi ditutup
    ...
    # [MODIFIKASI] Fungsi pembuatan arsip sekarang menggunakan thread dan progress bar
    ...
    def _on_archive_creation_error(self, message):
        ...
        if message == "Canceled":
            ...
            # Hapus file arsip yang mungkin dibuat sebagian
    ...
    # --- Fungsi-fungsi spesifik untuk setiap format arsip ---
    ...
    def _populate_table(self, items):
        ...
        for item in items:
            ...
            size_str = f"{item.get('size', 0):,}" if not item.get('is_dir') else ""
            mod_time = item.get('modified', "N/A")
            file_type = "Folder" if item.get('is_dir') else "File"
    ...
    # [BARU] Fungsi untuk membuka file yang di-double-click dari dalam arsip
    def _open_item_in_archive(self, item):
        row = item.row()
        file_type = self.table_widget.item(row, 3).text()
        if file_type == "Folder":
            return # Abaikan jika yang di-klik adalah folder
        ...
        try:
            # Ekstrak file tunggal ke direktori temporary
            self._extract_or_copy_selected(filenames=[filename], dest_folder=self.temp_dir, show_success_msg=False)
            
            extracted_file_path = os.path.join(self.temp_dir, filename)
            
            # Buka file menggunakan aplikasi default sistem operasi
    ...
    def _update_status_bar(self):
        # (Tidak ada perubahan di bagian ini)
        if not self.current_archive_path:
            self.status_bar.showMessage("Ready. Drag files here or use the toolbar.")
            return
    ...
    # [MODIFIKASI] Fungsi ekstraksi diubah untuk bisa dipanggil secara internal (tanpa dialog)
        ...
        try:
            ...
            if archive_type == '7z':
                ...
                with zipfile.ZipFile(archive_path, 'r') as archive:
                 ...
                 with tarfile.open(archive_path, 'r:*') as archive:
                    members_to_extract = [m for m in archive.getmembers() if m.name in filenames]
                    archive.extractall(path=dest_folder, members=members_to_extract)
            elif archive_type == 'rar' and RAR_SUPPORT:
                 with rarfile.RarFile(archive_path, 'r') as archive:
                    for f in filenames: archive.extract(f, path=dest_folder)
            ...
            if show_success_msg:
                ...
                raise e # Lemparkan lagi error jika dipanggil secara internal
    ...
    def _show_about(self):
        ...
        QMessageBox.information(
            ...
            "<b>Macan Archiver v2.2.0 (Multi-Format Edition)</b><br><br>"
