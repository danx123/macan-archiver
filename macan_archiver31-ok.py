...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QInputDialog, QCheckBox # Impor tambahan
)
...
SVG_ICONS = {
    ...
    "sfx": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="14.5 10.5 19.5 15.5"></polyline><polyline points="19.5 10.5 14.5 15.5"></polyline><path d="M8.5 15.5V11.5a1 1 0 0 1 1-1h1a1 1 0 0 1 1 1V13a1 1 0 0 1-1 1h-1a1 1 0 0 0-1 1v1.5a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1V15"></path></svg>""",
    ...
    "add": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="17" y1="11" x2="23" y2="11"></line></svg>""", # Ikon untuk "Add"
...
# --- [DIUBAH] ArchiveSettingsDialog ---
# Menghapus checkbox SFX
class ArchiveSettingsDialog(QDialog):
    ...
    def get_settings(self):
        ...
        if split_text in self.split_options:
            split_size = self.split_options[split_text]
        else:
            split_size = split_text.strip()
...
# --- [DIUBAH] ArchiveWorker ---
# Logika SFX dihapus total, worker hanya untuk arsip standar
class ArchiveWorker(QObject):
    ...
    def __init__(self, sevenzip_path, command, archive_path, source_paths, level, password, split_size):
        super().__init__()
    ...
    def run(self):
        try:
            self._create_standard_archive()
        except Exception as e:
            self.error.emit(f"An error occurred: {e}")

    def _create_standard_archive(self):
        self._execute_7z_command(self.archive_path)

    def _execute_7z_command(self, output_path):
        cmd = [self.sevenzip_path, self.command, output_path] + self.source_paths
        cmd.append(f"-mx={self.level}")
        if self.password:
            cmd.append(f"-p{self.password}")
        if self.split_size:
            if re.match(r'^\d+[kmgKMG]?$', self.split_size):
                cmd.append(f'-v{self.split_size}')
        cmd.append('-y')
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        self.sevenzip_path = self._find_7z_executable()
        self.sfx_builder_path = self._find_sfx_builder() # [BARU] Cari SFX builder
        ...
        # [DIUBAH] Peringatan jika SFX Builder tidak ditemukan
        if not self.sfx_builder_path:
             QMessageBox.warning(self, "SFX Builder Not Found",
                "7z SFX Builder.exe not found. Creating SFX archives via the toolbar button will be disabled.\n"
                "Please ensure '7z SFX Builder.exe' is in the application directory.")
    ...
    # --- [BARU] Metode untuk mencari 7z SFX Builder.exe ---
    def _find_sfx_builder(self):
        """Mencari path 7z SFX Builder.exe."""
        builder_name = "7z SFX Builder.exe"
        
        # Prioritas: direktori _MEIPASS jika dibundel dengan PyInstaller
        if hasattr(sys, "_MEIPASS"):
            candidate = os.path.join(sys._MEIPASS, builder_name)
            if os.path.exists(candidate):
                return candidate

        # Fallback: direktori yang sama dengan script/executable utama
        base_dir = os.path.dirname(sys.executable if getattr(sys, 'frozen', False) else __file__)
        candidate = os.path.join(base_dir, builder_name)
        if os.path.exists(candidate):
            return candidate
            
        # Fallback ke direktori kerja saat ini
        candidate = os.path.join(os.getcwd(), builder_name)
        if os.path.exists(candidate):
            return candidate

        return None

    # --- [DIUBAH] Metode disederhanakan untuk mencari 7z.exe saja ---
    def _find_7z_executable(self):
        ...
        # Cari 7z.exe dari PATH sistem
        path_from_env = shutil.which("7z")
        if path_from_env:
            return path_from_env

        # Cari di lokasi instalasi default
        for base_path in search_paths:
            if "Program Files" in base_path:
                path_candidate = os.path.join(base_path, "7-Zip", "7z.exe")
            else: # MEIPASS
                path_candidate = os.path.join(base_path, "7z.exe")
    ...
    # --- [DIUBAH] Fungsionalitas tombol SFX diubah ---
    def _create_menus_and_toolbars(self):
        self.create_action = QAction(self._get_themed_icon("create"), "&Create Archive...", self)
        self.sfx_action = QAction(self._get_themed_icon("sfx"), "Launch &SFX Builder...", self)
        self.sfx_action.setEnabled(bool(self.sfx_builder_path)) # Aktif jika builder ditemukan
        ...
        self.add_action = QAction(self._get_themed_icon("add"), "&Add Files...", self)
        ...
        self.sfx_action.triggered.connect(self._launch_sfx_builder) # [DIUBAH] Jalankan builder
        ...
        self.add_action.triggered.connect(self._add_to_archive)
        ...
        main_toolbar.addAction(self.sfx_action)
        main_toolbar.addAction(self.open_action)
        main_toolbar.addSeparator()
        main_toolbar.addAction(self.extract_action)
        main_toolbar.addAction(self.add_action)
        main_toolbar.addSeparator()
        main_toolbar.addAction(self.test_action)
        main_toolbar.addAction(self.copy_action)
        main_toolbar.addAction(self.delete_action)

    # --- [BARU] Metode untuk menjalankan 7z SFX Builder.exe ---
    def _launch_sfx_builder(self):
        if not self.sfx_builder_path:
            QMessageBox.critical(self, "Error", "7z SFX Builder.exe could not be found.")
            return
        
        try:
            # [DIUBAH] Gunakan ctypes untuk meminta hak akses Administrator (elevation)
            ret = ctypes.windll.shell32.ShellExecuteW(
                None,           # Handle ke window parent
                "runas",        # Operasi: jalankan sebagai admin
                self.sfx_builder_path, # Path ke file exe
                None,           # Parameter tambahan (tidak ada)
                None,           # Direktori kerja (default)
                1               # Tampilkan window-nya
            )

            # ShellExecuteW mengembalikan nilai > 32 jika berhasil
            if ret > 32:
                self.status_bar.showMessage("Launched 7z SFX Builder.", 5000)
            else:
                # Jika user menekan "No" di jendela UAC, ret akan <= 32
                QMessageBox.warning(self, "Operation Canceled", "The SFX builder was not launched because administrator rights were not granted.")
                self.status_bar.showMessage("SFX builder launch canceled by user.", 5000)

        except Exception as e:
            QMessageBox.critical(self, "Failed to Launch", f"Could not start 7z SFX Builder.exe:\n{e}")
            self.status_bar.showMessage("Failed to launch SFX builder.", 5000)
    ...
    # --- [DIUBAH] _perform_archive_creation disederhanakan tanpa logika SFX ---
    def _perform_archive_creation(self, source_paths, update_mode=False):
        ...
        if update_mode:
            ...
            file_filter = "Macan Archive (*.mcn);;7z Archives (*.7z)"
            default_ext = ".mcn"

            save_path, _ = QFileDialog.getSaveFileName(self, "Save Archive As", os.path.join(self.current_directory, default_name), file_filter)
        ...
        self.worker = ArchiveWorker(self.sevenzip_path, command, save_path, source_paths, level, password, split_size)
    ...
    def _on_archive_operation_finished(self, save_path):
        ...
        # Langsung buka arsip yang baru dibuat
        self._list_archive_contents(save_path, force_reload=True)
        self.thread.quit()
    ...
    def _extract_archive(self, selected_only=False, copy_mode=False):
        ...
        self.status_bar.showMessage("Extracting, please wait...")
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)

        try:
            cmd = [self.sevenzip_path, 'x', self.current_archive_path, f'-o{dest_folder}']
            if password:
            ...
            if result.returncode == 0:
                ...
                raise RuntimeError(result.stderr or "An unknown error occurred with 7-Zip.")
    ...
    def _show_about(self):
        QMessageBox.information(self, "About Macan Archiver",
            "<b>Macan Archiver v4.2.5 (Shell Integration/SFX Builder)</b><br><br>"
            "A professional file archiver powered by the 7-Zip engine. 🧭<br>"
            "<b>Supported Formats via 7-Zip:</b> .mcn, .7z, .zip, .tar, .rar, and more.<br>"
            "Features external SFX creation via the 'SFX' toolbar button.<br><br>"
    ...
    def _show_help_content(self):
        ...
        <ul>
            <li><b>Creating Archives:</b> Click the 'Create' icon or use File > Create Archive. You can select multiple files or a single folder.</li>
            <li><b>Creating SFX Archives:</b> Click the 'SFX' icon to launch the external '7z SFX Builder' application. This is useful for distributing files to users who may not have an archiver installed.</li>
            <li><b>Opening Archives:</b> Double-click a supported archive file, drag & drop it onto the window, or use the 'Open' icon.</li>
            <li><b>Adding to Archives:</b> While an archive is open, click the 'Add' icon to add more files or folders to the existing archive.</li>
            ...
            <li><b>Password & Splitting:</b> In the settings dialog, you can set an optional password and split the archive into smaller volumes using the dropdown menu.</li>
        </ul>
        ...
        <p>To enable, run Macan Archiver as an administrator and go to Help > Shell Integration > Register Context Menu. This will add "Open with Macan Archiver" and "Add to archive (Macan)..." to your right-click menu in Windows Explorer.</p>
        """
        dialog = QDialog(self)
    ...
    def _update_shell_integration(self, register=True):
        ...
        try:
            # Tentukan path untuk executable modular
            if getattr(sys, 'frozen', False): # Jika dijalankan sebagai .exe
                app_dir = os.path.dirname(sys.executable)
            else: # Jika dijalankan sebagai script .py
                app_dir = os.path.dirname(os.path.abspath(__file__))
            
            add_exe_path = os.path.join(app_dir, "MacanAdd.exe")
            extract_exe_path = os.path.join(app_dir, "MacanExtract.exe")

            # Cek jika executable tersebut ada sebelum mendaftar
            if register and (not os.path.exists(add_exe_path) or not os.path.exists(extract_exe_path)):
                 QMessageBox.warning(self, "Missing Components",
                    "Could not find MacanAdd.exe or MacanExtract.exe in the application directory.\n\n"
                    "Please ensure all components are present to register the shell menu.")
                 return

            add_command = f'"{add_exe_path}" %*'
            extract_command = f'"{extract_exe_path}" "%1"'

            # Registrasi untuk menambah file/folder ke arsip
            add_keys = {
                r'*\shell\MacanAddToArchive': "Add to archive (Macan)...",
                r'Directory\shell\MacanAddToArchive': "Add to archive (Macan)..."
            }
            for key_path, value in add_keys.items():
                full_key_path_cmd = fr'{key_path}\command'
                if register:
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, key_path) as key:
                        winreg.SetValue(key, None, winreg.REG_SZ, value)
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd) as cmd_key:
                        winreg.SetValue(cmd_key, None, winreg.REG_SZ, add_command)
                else: # Unregister
                    try:
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd)
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, key_path)
                    except FileNotFoundError: pass

            # Registrasi untuk mengekstrak arsip
            archive_exts = ['.7z', '.mcn', '.zip', '.rar', '.tar', '.gz', '.bz2', '.xz']
            extract_key_name = r'shell\MacanExtract'
            for ext in archive_exts:
                key_path = fr'SystemFileAssociations\{ext}\{extract_key_name}'
                full_key_path_cmd = fr'{key_path}\command'
                if register:
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, key_path) as key:
                        winreg.SetValue(key, None, winreg.REG_SZ, "Extract with Macan...")
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd) as cmd_key:
                        winreg.SetValue(cmd_key, None, winreg.REG_SZ, extract_command)
                else: # Unregister
                     try:
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd)
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, key_path)
            ...
            # Registrasi untuk membuka arsip dengan aplikasi utama
            open_key_name = r'shell\OpenWithMacanArchiver'
            main_exe_path = sys.executable
            if main_exe_path.lower().endswith(("python.exe", "pythonw.exe")):
                script_path = os.path.abspath(__file__)
                open_command = f'"{main_exe_path}" "{script_path}" "%1"'
            else:
                open_command = f'"{main_exe_path}" "%1"'

            for ext in archive_exts:
                key_path = fr'SystemFileAssociations\{ext}\{open_key_name}'
                full_key_path_cmd = fr'{key_path}\command'
                if register:
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, key_path) as key:
                        winreg.SetValue(key, None, winreg.REG_SZ, "Open with Macan Archiver")
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd) as cmd_key:
                        winreg.SetValue(cmd_key, None, winreg.REG_SZ, open_command)
                else: # Unregister
                    try:
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, full_key_path_cmd)
                        winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, key_path)
