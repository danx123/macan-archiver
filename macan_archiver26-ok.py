...
class ArchiveWorker(QObject):
    ...
    def __init__(self, sevenzip_path, command, archive_path, source_paths, level, password):
        ...
        self._is_running = True  # Flag untuk membatalkan proses
        self.process = None      # Simpan referensi proses subprocess
...
class MacanArchiver(QMainWindow):
    ...
    def _perform_archive_creation(self, source_paths, update_mode=False):
        ...
        if update_mode:
            ...
            # Otomatis tambahkan ekstensi jika belum ada
            if not save_path.lower().endswith(('.mcn', '.7z')):
                save_path += '.mcn'
        
        # --- KODE BARU: Setup dan tampilkan Progress Dialog ---
        ...
        self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal) # Blokir window utama
        self.progress_dialog.setAutoClose(False) # Jangan tutup otomatis
        self.progress_dialog.setAutoReset(False) # Jangan reset otomatis

        # Setup Thread dan Worker
        self.thread = QThread()
        command = 'u' if update_mode else 'a' # 'u' untuk update/add, 'a' untuk create new
        self.worker = ArchiveWorker(self.sevenzip_path, command, save_path, source_paths, level, password)
        self.worker.moveToThread(self.thread)

        # Hubungkan sinyal dari worker ke slot/fungsi
        self.progress_dialog.canceled.connect(self.worker.stop) # Tombol cancel -> hentikan worker
        ...
        self.worker.progress.connect(self.progress_dialog.setValue) # Update progress bar
        self.worker.progress_text.connect(self.progress_dialog.setLabelText) # Update teks label

        # Mulai thread dan tampilkan dialog
        self.thread.start()
