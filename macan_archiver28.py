...
# Mengganti input split menjadi dropdown dan menambahkan opsi force_sfx
class ArchiveSettingsDialog(QDialog):
    def __init__(self, parent=None, force_sfx=False):
        ...
        # Opsi SFX
        self.sfx_check = QCheckBox("Create SFX archive (GUI .exe)")
        layout.addRow("", self.sfx_check)
        if force_sfx:
            self.sfx_check.setChecked(True)
            self.sfx_check.setEnabled(False)

        # --- [BARU] Mengganti QLineEdit menjadi QComboBox untuk Split ---
    ...
    def get_settings(self):
        ...
        is_sfx = self.sfx_check.isChecked()
        
        # --- [BARU] Logika untuk mendapatkan ukuran split dari QComboBox ---
        split_text = self.split_combo.currentText()
        if split_text in self.split_options:
            split_size = self.split_options[split_text]
        else:
            split_size = split_text.strip() # Ambil nilai kustom
        
        return level, password, is_sfx, split_size
...
# Logika diubah untuk menangani pembuatan SFX GUI
class ArchiveWorker(QObject):
    ...
    def __init__(self, sevenzip_path, sfx_module_path, command, archive_path, source_paths, level, password, is_sfx, split_size):
        super().__init__()
        self.sevenzip_path = sevenzip_path
        self.sfx_module_path = sfx_module_path # [BARU] Path ke 7zS.sfx
    ...
    def _create_gui_sfx(self):
        if not self.sfx_module_path:
            self.error.emit("GUI SFX module (7zS.sfx) not found.")
            return

        # 1. Buat arsip .7z temporer
        temp_archive_path = None
        try:
            fd, temp_archive_path = tempfile.mkstemp(suffix=".7z")
            os.close(fd)
            
            # Gunakan logika pembuatan arsip standar untuk membuat file temporer
            success = self._execute_7z_command(temp_archive_path)
            if not success:
                # Error sudah di-emit oleh _execute_7z_command
                return
            ...
            # 2. Buat file config untuk SFX GUI
            self.progress_text.emit("Creating SFX configuration...")
            sfx_config_content = (
                ';!@Install@!UTF-8!\n'
                'Title="Macan Archiver Self-Extractor"\n'
                'BeginPrompt="Do you want to extract this archive?"\n'
                'RunProgram="setup.exe"\n' # Contoh, bisa dikosongkan
                ';!@InstallEnd@!'
            )
            fd, config_path = tempfile.mkstemp(suffix=".txt")
            with os.fdopen(fd, 'w', encoding='utf-8') as f:
                f.write(sfx_config_content)

            # 3. Gabungkan: 7zS.sfx + config.txt + archive.7z -> final.exe
            self.progress_text.emit("Assembling final SFX executable...")
            with open(self.archive_path, 'wb') as f_out:
                with open(self.sfx_module_path, 'rb') as f_sfx:
                    shutil.copyfileobj(f_sfx, f_out)
                with open(config_path, 'rb') as f_cfg:
                    shutil.copyfileobj(f_cfg, f_out)
                with open(temp_archive_path, 'rb') as f_arch:
    ...
    def _execute_7z_command(self, output_path):
        ...
        if self.split_size and not self.is_sfx: # SFX tidak mendukung split
        ...
        if return_code == 0:
            if not self.is_sfx: # Jika SFX, finish akan di-emit nanti
                self.finished.emit(self.archive_path)
            return True
        else:
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        self.sevenzip_path, self.sfx_module_path = self._find_7z_executables() # Cari 7z.exe dan 7zS.sfx
        ...
        if not self.sfx_module_path:
             QMessageBox.warning(self, "SFX Module Not Found",
                "GUI SFX module (7zS.sfx) not found. Creating SFX archives will be disabled.\n"
                "Please ensure 7zS.sfx is in the same directory as 7z.exe.")
    ...
    # --- [DIUBAH] Mencari 7z.exe dan 7zS.sfx ---
    def _find_7z_executables(self):
        """Mencari path 7z.exe dan 7zS.sfx."""
        sevenzip_path = None
        sfx_path = None
        search_paths = []
        ...
        if path_from_env:
            sevenzip_path = path_from_env
            base_dir = os.path.dirname(path_from_env)
            sfx_candidate = os.path.join(base_dir, "7zS.sfx")
            if os.path.exists(sfx_candidate):
                sfx_path = sfx_candidate
            return sevenzip_path, sfx_path
        ...
        for base_path in search_paths:
            ...
            if os.path.exists(path_candidate):
                sevenzip_path = path_candidate
                sfx_candidate = os.path.join(os.path.dirname(path_candidate), "7zS.sfx")
                if os.path.exists(sfx_candidate):
                    sfx_path = sfx_candidate
                return sevenzip_path, sfx_path
    ...
    def _update_all_icons(self):
        ...
        self.sfx_action.setIcon(self._get_themed_icon("sfx")) # [BARU]
    ...
    # --- [DIUBAH] Menambah action dan tombol SFX ---
    def _create_menus_and_toolbars(self):
        self.create_action = QAction(self._get_themed_icon("create"), "&Create Archive...", self)
        self.sfx_action = QAction(self._get_themed_icon("sfx"), "Create &SFX Archive...", self) # [BARU]
        self.sfx_action.setEnabled(bool(self.sfx_module_path)) # Nonaktifkan jika 7zS.sfx tidak ada
        ...
        self.sfx_action.triggered.connect(self._create_sfx_archive) # [BARU]
        ...
        main_toolbar.addAction(self.sfx_action) # [BARU] Menambahkan tombol SFX
    ...
    # --- [DIUBAH] _perform_archive_creation menangani mode SFX ---
    def _perform_archive_creation(self, source_paths, update_mode=False, sfx_mode=False):
        ...
        if update_mode:
            ...
            settings_dialog = ArchiveSettingsDialog(self, force_sfx=sfx_mode)
            if not settings_dialog.exec():
                self.status_bar.showMessage("Canceled archive creation.")
                return
            
            level, password, is_sfx, split_size = settings_dialog.get_settings()
            
            if is_sfx:
                file_filter = "Self-Extracting Archive (*.exe)"
                default_ext = ".exe"
            else:
            ...
            if is_sfx and not save_path.lower().endswith('.exe'):
                save_path += '.exe'
            elif not is_sfx and not save_path.lower().endswith(('.mcn', '.7z')):
        ...
        # [DIUBAH] Kirim path SFX module ke Worker
        self.worker = ArchiveWorker(self.sevenzip_path, self.sfx_module_path, command, save_path, source_paths, level, password, is_sfx, split_size)
    ...
    def _on_archive_operation_finished(self, save_path):
        ...
        # Jangan buka file .exe sebagai arsip, kembali ke browser direktori
        if save_path.lower().endswith('.exe'):
            self._browse_to_path(os.path.dirname(save_path))
        else:
    ...
    # --- [DIUBAH] Metode _create_archive dan _create_sfx_archive ---
    def _create_archive(self):
        self._prompt_for_sources_and_create(sfx_mode=False)

    def _create_sfx_archive(self):
        self._prompt_for_sources_and_create(sfx_mode=True)

    def _prompt_for_sources_and_create(self, sfx_mode=False):
        ...
        self._perform_archive_creation(source, sfx_mode=sfx_mode)
    ...
    def _show_about(self):
        QMessageBox.information(self, "About Macan Archiver",
            "<b>Macan Archiver v4.1.0 (GUI SFX Edition)</b><br><br>"
    ...
    def _show_help_content(self):
        ...
        <ul>
            ...
            <li><b>Creating SFX Archives:</b> Click the 'SFX' icon to create a graphical self-extracting archive (.exe). This is useful for distributing files to users who may not have an archiver installed.</li>
