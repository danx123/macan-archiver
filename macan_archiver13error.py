...
QMessageBox QLabel, QDialog QLabel, QDialog QComboBox, QDialog QProgressBar, QDialog QCheckBox { color: #e0e0e0; }
...
class ArchiveWorker(QObject):
    ...
    def __init__(self, source_paths, save_path, filters, password=None):
        ...
        self.password = password # [BARU]
        self._is_running = True

    def run(self):
        try:
            # [MODIFIKASI] Menambahkan password ke SevenZipFile
    ...
    def run(self):
        try:
            ...
            if self.archive_type == '7z':
                # [MODIFIKASI] Menambahkan password
                with py7zr.SevenZipFile(self.archive_path, 'r', password=self.password) as archive:
                    ...
                    for f in files_to_extract:
                        if not self._is_running: break
                        self.progress_text.emit(f"Extracting: {os.path.basename(f)}...")
                        archive.extract(targets=[f], path=self.dest_folder)
                        processed_files += 1
                        self.progress.emit(processed_files)
            elif self.archive_type == 'zip':
                # [MODIFIKASI] Menambahkan password (dalam bentuk bytes)
                ...
                with zipfile.ZipFile(self.archive_path, 'r') as archive:
                    ...
                    for f in files_to_extract:
                        if not self._is_running: break
                        self.progress_text.emit(f"Extracting: {os.path.basename(f)}...")
                        archive.extract(f, path=self.dest_folder, pwd=pwd_bytes)
                        processed_files += 1
                        self.progress.emit(processed_files)
                ...
                with rarfile.RarFile(self.archive_path, 'r', pwd=self.password) as archive:
                    ...
                    for f in files_to_extract:
                        if not self._is_running: break
                        self.progress_text.emit(f"Extracting: {os.path.basename(f)}...")
                        archive.extract(f, path=self.dest_folder)
                        processed_files += 1
                        self.progress.emit(processed_files)
...
class MacanArchiver(QMainWindow):
    def __init__(self, file_to_open=None):
        ...
        self.current_archive_password = None # [BARU]
    ...
    def _create_menus_and_toolbars(self):
        ...
        self.help_content_action = QAction("&Help Content", self) # [BARU]
        ...
        self.help_content_action.triggered.connect(self._show_help_content) # [BARU]
        ...
        help_menu.addAction(self.help_content_action) # [BARU]
    ...
    def _perform_archive_creation(self, source_paths):
        ...
        self.thread = QThread()
        self.worker = ArchiveWorker(source_paths, save_path, filters, password) # [MODIFIKASI] Pass password
    ...
    def _list_archive_contents(self, file_path, force_reload=False, password_attempt=None):
        ...
        try:
            ...
            self.current_archive_password = password_attempt # [BARU] Simpan password yg berhasil
        ...
        except (py7zr.PasswordRequired, RuntimeError, rarfile.BadRarFile, zipfile.BadZipFile) as e:
            # [BARU] Tangani error password
    ...
    def _list_zip_contents(self, file_path, password=None):
        ...
        with zipfile.ZipFile(file_path, 'r') as archive:
            if archive.testzip() is not None: # Quick check, may trigger password error
                pass
    ...
    def _perform_extraction(self, targets=None, dest_folder=None, show_success_msg=True):
        ...
        self.thread = QThread()
        # [MODIFIKASI] Pass password to worker
    ...
    # [BARU] Menampilkan dialog bantuan
    def _show_help_content(self):
        ...
        """
        QMessageBox.information(self, "Help Content", help_text)

    # [PERBAIKAN TOTAL] Fungsi untuk integrasi shell
    def _update_shell_integration(self, register=True):
        ...
        try:
            ...
            # Jika dijalankan sebagai script python, gunakan path script. Jika sudah jadi .exe, gunakan path .exe
            ...
            # Definisikan semua perintah dan di mana mereka harus didaftarkan
            archive_exts = ['.7z', '.mcn', '.zip', '.rar', '.tar', '.gz', '.bz2', '.xz']
            
            # 1. Perintah untuk file arsip
            archive_commands = {
            ...
            # 2. Perintah untuk semua file dan folder (Add to Archive)
            add_to_archive_cmd = f'{base_command} --add-to-archive %*' # %* untuk menangani multiple files
            target_keys = {
...
def handle_cli_add_to_archive(source_paths):
    ...
    # Gunakan instance MacanArchiver dummy hanya untuk memanggil logikanya
...
def handle_cli_extract(archive_path, extract_here=False):
    ...
    # Buat worker langsung, bypass UI utama
    ...
    # Coba ekstrak tanpa password dulu
    password = None
    while True:
        try:
            worker = ExtractionWorker(archive_type, archive_path, dest_folder, password=password)
            worker.run() # Jalankan secara sinkron untuk CLI
            if "Successfully" in worker.finished.emit():
                QMessageBox.information(None, "Success", f"Successfully extracted to {dest_folder}")
            break
        except (py7zr.PasswordRequired, RuntimeError, rarfile.BadRarFile) as e:
            dialog = PasswordDialog(filename=archive_path)
            if dialog.exec():
                password = dialog.get_password()
            else:
                print("Password not provided. Extraction canceled.")
                break # Batalkan jika user cancel dialog password
        except Exception as e:
            QMessageBox.critical(None, "Extraction Error", f"An error occurred: {e}")
            break
...
if __name__ == "__main__":
    ...
    # [BARU] Argumen untuk integrasi shell
    ...
    # Argumen untuk membuka file dari shell
    parser.add_argument('file_path_arg', nargs='?', default=None, help='File to open directly in the GUI.')
    
    args, unknown = parser.parse_known_args() # Gunakan parse_known_args untuk fleksibilitas

    # Prioritaskan argumen dari shell integration
    ...
    # Jika tidak ada argumen CLI yang dieksekusi, jalankan GUI
    app = QApplication(sys.argv)
    
    if not RAR_SUPPORT:
        print("Warning: 'rarfile' module not found. RAR file support will be disabled.")

    # Tentukan file yang akan dibuka di GUI
    file_to_open = args.file_path_arg
    if not file_to_open and unknown:
        # Menangani kasus di mana file path tidak dikenali sebagai argumen positional
        # (misal: "macan_archiver.py C:\path\to\file.zip")
