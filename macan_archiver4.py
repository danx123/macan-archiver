...
class MacanArchiver(QMainWindow):
    ...
    def _setup_ui(self):
        # [DRAG & DROP] Aktifkan penerimaan event drop pada main window
    ...
    # [DRAG & DROP] Event handler saat file diseret masuk ke jendela
    def dragEnterEvent(self, event):
        # Cek apakah data yang diseret berisi URL file
        if event.mimeData().hasUrls():
            event.acceptProposedAction() # Terima event ini
        else:
            event.ignore() # Tolak event ini

    # [DRAG & DROP] Event handler saat file dilepaskan di jendela
    def dropEvent(self, event):
        urls = event.mimeData().urls()
        if not urls:
            return

        # Ambil path file lokal dari URL yang didrop
        paths = [url.toLocalFile() for url in urls]

        # KASUS 1: Jika hanya satu file yang didrop dan itu adalah .mcn, buka arsip
        if len(paths) == 1 and paths[0].lower().endswith(('.mcn', '.7z')):
            self._list_archive_contents(paths[0])
        # KASUS 2: Jika file lain atau banyak file/folder, mulai proses pembuatan arsip
        else:
    ...
    # [DRAG & DROP] Modifikasi fungsi _create_archive.
    # Fungsi ini sekarang hanya akan membuka dialog untuk memilih file/folder.
    # Logika inti pembuatan arsip dipindahkan ke _perform_archive_creation.
    def _create_archive(self):
        # Coba pilih beberapa file terlebih dahulu
        files_to_add, _ = QFileDialog.getOpenFileNames(self, "Select Files to Archive")
        source = files_to_add
        
        # Jika tidak ada file yang dipilih, coba pilih satu folder
        if not files_to_add:
            ...
            if not folder_to_add:
                ...
                return
            source = [folder_to_add] # Jadikan list agar konsisten
        
        # Panggil fungsi inti untuk membuat arsip
        self._perform_archive_creation(source)

    # [DRAG & DROP] Fungsi baru yang berisi logika utama untuk membuat arsip.
    # Bisa dipanggil dari _create_archive (klik tombol) atau dari dropEvent (drag-drop).
    def _perform_archive_creation(self, source_paths):
        ...
        try:
            with py7zr.SevenZipFile(save_path, 'w', filters=filters) as archive:
                # Loop melalui semua path sumber (bisa file atau folder)
    ...
    def _show_about(self):
        QMessageBox.information(
            ...
            "<b>Macan Archiver v2.1.0 (Drag & Drop Edition)</b><br><br>"
...
f"Now with Drag & Drop support! 📂<br>"
